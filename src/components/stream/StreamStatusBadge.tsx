...
const BADGE_STYLES = {
  LIVE: {
    container:
      'bg-red-600/10 backdrop-blur-sm flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] w-fit border-2 border-red-500',
  ...
  SCHEDULED: {
    container:
      'bg-scheduled-badge-bg backdrop-blur-sm flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] w-fit border-2 border-scheduled-badge-border',
    icon: 'size-3 text-scheduled-badge-text',
    text: 'text-[10px] text-scheduled-badge-text font-bold tracking-wide uppercase',
  },
  LOCK: {
    container:
      'bg-amber-600/10 backdrop-blur-sm flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] w-full border-2 border-amber-500',
    icon: 'size-3 text-amber-400',
    text: 'text-[10px] text-amber-400 font-bold tracking-wide uppercase',
  },
  ENDED: {
    container:
      'flex items-center gap-2 px-3 py-1.5 bg-badge-ended/90 backdrop-blur-sm rounded-md text-xs font-bold uppercase tracking-wider shadow-lg',
  },
} as const;
...
export const StreamStatusBadge = ({
  ...
  if (status === StreamStatus.SCHEDULED) {
    const ariaLabel = scheduledStartTime
      ? `Live: ${formatDate(scheduledStartTime)} at ${formatTime(scheduledStartTime)}`
      : 'Live: TBA';
    ...
    return (
      ...
      >
        ...
        <span className={BADGE_STYLES.SCHEDULED.text}>
          {scheduledStartTime ? (
            <>
              Upcoming: {multiline && <br />}
              {formatDate(scheduledStartTime)} at {formatTime(scheduledStartTime)}
            </>
          ) : (
            'Upcoming: TBA'
          )}
  ...
  if (status === 'lock') {
    const ariaLabel = lockDate ? `Picks Lock: ${lockDate}` : 'Picks Lock: TBA';
    ...
    return (
      ...
      >
        ...
        <span className={BADGE_STYLES.LOCK.text}>Picks Lock: {lockDate || 'TBA'}</span>
      </motion.div>
    );
  }
  ...
  if (status === StreamStatus.ENDED) {
    return (
      <motion.div {...ANIMATION_CONFIG} role="status" aria-label="Stream ended">
        <span className={BADGE_STYLES.ENDED.container}>Ended</span>
      </motion.div>
    );
  }
