...
export default function BetCard(props: BetCardType) {
  ...
  return (
    <Card className="h-full flex flex-col bg-zinc-900 border border-border shadow-lg overflow-hidden">
      <CardHeader className="p-4 pb-0 flex flex-row gap-3 items-center">
        ...
        <CardTitle onClick={handleClick} className="text-md cursor-pointer hover:underline line-clamp-2">{props.name}</CardTitle>
      </CardHeader>
      <CardContent className="flex-1 flex flex-col gap-2 p-4 justify-center">
        ...
        {props.options.slice(0, 2).map((option, i) => (
          <div key={i} onClick={handleClick} className="flex-1 flex gap-4 items-center justify-between hover:bg-zinc-600 transition-all cursor-pointer px-2 py-1 rounded-md">
            <div className={cn("text-sm rounded-full font-semibold", option.selected && "text-[#BDFF00]")}>{option.option}</div>
            <div className="text-lg font-semibold flex">{option.percentage}%</div>
          </div>
        ))}
        {props.options.length > 2 &&
          <div onClick={handleClick} className="flex-1 flex gap-4 items-center justify-between hover:bg-zinc-600 transition-all cursor-pointer px-2 py-1 rounded-md">
            <div className="text-sm rounded-full font-semibold">{props.options.length - 2} more...</div>
          </div>
        }
      </CardContent>
