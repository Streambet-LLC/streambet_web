...
function getActiveRoundIndex(betData) {
  ...
  for (const status of statusPriority) {
    const idx = betData.findIndex(r => (r.status || '').toLowerCase().includes(status));
    if (idx !== -1) return idx;
  }
  // do nothing
  return;
...
export const AdminBettingRoundsCard = ({
  ...
  return (
    <div className="flex flex-col items-center w-full mt-4">
      ...
      >
        {/* Card Header */}
        <CardHeader className="flex flex-row items-center justify-between h-[50px] rounded-t-xl px-6 py-0 bg-[#000000B2]">
                        ...
                        onClick={async () => {
                          ...
                          if (errorIndices.length > 0) {
                            ...
                            setTimeout(() => {
                              const el = document.querySelector(
                                '[data-round-index="' + errorIndices[0] + '"]'
                              );
                          ...
                          if (validationErrors.length > 0) {
                            ...
                            setTimeout(() => {
                              ...
                              const el = document.querySelector(
                                '[data-round-index="' + first.roundIndex + '"]'
                              );
                              if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                            }, 500);
                            toast({
                          ...
                          try {
                            ...
                            await api.admin.updateBettingData({
                            ...
                            toast({
                              title: 'Error',
                              description: getMessage(e) || 'Failed to update pick',
                              variant: 'destructive',
                            });
                          }
              ...
              <CarouselContent className="flex items-center !ml-0 md:-ml-4">
                {rounds && rounds?.length > 0 ? (
                  rounds.map((round, idx) => {
                    ...
                    const isActive = isLocked || isOpen || isCreated;
                    return (
                      <CarouselItem
                          ...
                          {isActive && isOpen && (
                            <div className="flex flex-col items-center w-full mt-2">
                              <div
                                ...
                                style={{
                                  fontFamily: 'FabioXM, Inter, sans-serif',
                                  fontWeight: 700,
                                }}
                              >
                          ...
                          {isActive && isLocked && (
                            <div className="flex flex-col items-center w-full mt-2">
                              ...
                              <div className="flex flex-wrap gap-2 justify-center w-full max-h-16 overflow-y-auto mb-2 winner-scrollbar px-2">
                                {round.options.map(opt => {
                                  const isSelected =
                                    selectedOption[round.roundId] === (opt.id || opt.optionId);
                                  return (
                                    <button
                                      key={opt.id || opt.optionId || opt.option}
                                      ...
                                      onClick={() =>
                                        setSelectedOption(prev => ({
                                          ...prev,
                                          [round.roundId]: opt.id || opt.optionId,
                                        }))
                                      }
                                      title={opt.option}
                                    >
                            ...
                            <div className="flex flex-col items-center w-full mt-2">
                              ...
                              (isSweepCoins
                                ...
                                <div className="flex flex-col items-center w-full">
                                    ...
                                    {(isSweepCoins
                                      ...
                                      >
                                        <Avatar className="h-10 w-10 mb-1">
                                          <AvatarImage
                                            src={getImageLink(winner?.avatar)}
                                            alt={winner?.userName}
                                          />
                                          <AvatarFallback>
                                  ...
                                  <div
                                    className="flex items-center mt-1 text-[12px] px-2"
                                    style={FabioBoldStyle}
                                  >
