...
export const TotalPot = ({ streamId, initialTotalBets = 0, className = '' }: TotalPotProps) => {
  ...
  useEffect(() => {
    ...
    if (totalPot !== undefined && totalPot !== null && totalPot !== previousValueRef.current) {
      ...
      const significantChange =
        Math.abs((totalPot - stableValueRef.current) / Math.max(stableValueRef.current, 1)) > 0.05;
      ...
      if (significantChange || totalPot === 0) {
        ...
        }, 200); // Slightly longer delay for more stability
      }
    }
  ...
  // Force refresh the total pot frequently for better syncing
  useEffect(() => {
    const intervalId = setInterval(() => {
      refetchTotalPot();
    }, 1500); // Refresh interval
