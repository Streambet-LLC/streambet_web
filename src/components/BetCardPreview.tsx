...
export default function BetCardPreview(props: BetCardType) {
  ...
  return (
    <motion.div whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}>
      <Card className={`h-full flex flex-col border border-gray-600 shadow-lg overflow-hidden`}>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-2">
                <CardTitle className={cn('text-md line-clamp-2')}>{props.name}</CardTitle>
              </div>
            </div>
          ...
          {props.options.length === 3 ? (
            <>
              {props.options.map((option, i) => (
                ...
                >
                  <div className={cn('text-sm rounded-full font-semibold')}>{option.option} </div>
