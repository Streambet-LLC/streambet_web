...
export async function verifyUserLocation(): Promise<GeolocationResult> {
  // COMMENTED OUT: Geolocation blocking functionality
  // try {
  ...
  // Always allow access when geolocation is disabled
  return {
    allowed: true,
    error: 'Geolocation checking is disabled.',
  };
}
...
export function getBrowserGeolocation(): Promise<GeolocationPosition> {
  // COMMENTED OUT: Browser geolocation functionality
  ...
  return Promise.reject(new Error('Browser geolocation is disabled'));
}
