...
async function getPayPalAccessToken() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get PayPal access token: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
async function createPayPalPayout(email: string, amount: number, accessToken: string) {
  try {
    console.log(`Creating PayPal payout for ${email} with amount ${amount}`);
    const payoutData = {
      sender_batch_header: {
    ...
    console.log('PayPal payout request data:', JSON.stringify(payoutData));
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to create PayPal payout: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
Deno.serve(async req => {
  ...
  try {
    ...
    console.log(`Received request for amount: ${amount}, email: ${email}`);
